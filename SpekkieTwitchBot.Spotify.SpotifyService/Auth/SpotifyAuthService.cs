...
public class SpotifyAuthService(SpotifyFileReader spotifyFileReader, Logger logger)
{
    ...
    /*public async Task<SpotifyAuth> FixAuth(SpotifyAuth spotifyAuth)
    {
        ...
        string authUrl = GetAuthorizationUrl(scopes, spotifyAuth.ClientId, "http://localhost:4202");
    ...
    private async Task<SpotifyAuth> ExchangeCodeForTokensAsync(string authCode, SpotifyAuth spotifyAuth)
    {
        ...
        var requestBody = new StringContent(
            $"grant_type=authorization_code&code={authCode}&redirect_uri={Uri.EscapeDataString("http://localhost:4202")}",
...
public class SpotifyAuthService(SpotifyFileReader spotifyFileReader, Logger logger)
{
    ...
    public async Task<SpotifyAuth> FixAuth(SpotifyAuth spotifyAuth)
    {
        ...
        spotifyAuth = await ExchangeCodeForTokensAsync(authCode, spotifyAuth);
        Console.WriteLine($"Access Token: {spotifyAuth.Token}");
        Console.WriteLine($"Refresh Token: {spotifyAuth.RefreshToken}");#1#
