...
public class TwitchWebsocketService : IHostedService
{
        ...
        )
    {
        ...
        _twitchUserAuth = twitchAuthService.GetTwitchUserAuth() ?? throw new ArgumentNullException(nameof(_twitchUserAuth));
        _generalTwitchAuth = twitchAuthService.GetGeneralTwitchAuth() ?? throw new ArgumentNullException(nameof(_generalTwitchAuth));
    ...
    private void SetupTwitchClient()
    {
        ...
        _CustomTwitchClient.R9KMode += R9KMode;
    ...
    private void SetupPubSub()
    {
        ...
        _CustomPubsub.R9KBeta += R9KBeta;
        _CustomPubsub.R9KBetaOff += R9KBetaOff;
    ...
    {
        _GeneralLogger.LogInfo($"New gifted subscription: {e.GiftedSubscription.DisplayName} has just been gifted a subscription");
    }
    
    private void OnCommunitySubscription(object? sender, OnCommunitySubscriptionArgs e)
    {
        _GeneralLogger.LogInfo($"New community subscription: {e.GiftedSubscription.DisplayName} just got gifted a subscription");
    }
    
    private void OnContinuedGiftedSubscription(object? sender, OnContinuedGiftedSubscriptionArgs e)
    {
        _GeneralLogger.LogInfo($"{e.ContinuedGiftedSubscription.DisplayName} continued their gifted subscription to your channel");
    }
    
    private void OnExistingUsersDetected(object? sender, OnExistingUsersDetectedArgs e)
    {
        _GeneralLogger.LogInfo($"Existing user detected: {e.Users.ToList()}");
    }
    ...
    {
        _GeneralLogger.LogInfo($"{e.UserTimeout.Username} timed out for {e.UserTimeout.TimeoutReason} for the duration {e.UserTimeout.TimeoutDuration} seconds");
    }
    ...
    {
        _GeneralLogger.LogInfo($"User {e.TimedoutUser} has been timed out for {e.TimeoutDuration} for {e.TimeoutReason}");
    }
    ...
    {
        _GeneralLogger.LogInfo($"Bits received: {e.BitsUsed} cheered by {e.Username}, they have cheered {e.TotalBitsUsed} in {e.ChannelName}");
    }
    ...
    {
        _GeneralLogger.LogInfo($"Channel subscription: {e.Subscription.Username} has subscribed for {e.Subscription.Months}");
    }
